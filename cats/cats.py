...
def about(subject):
    ...
    def select(paragraph):
        ...
        return any(p in subject_set for p in p_list)
...
def feline_fixes(typed, source, limit):
    ...
    count = 0 if typed[0] == source[0] else 1
    return count + feline_fixes(typed[1:], source[1:], limit - count)
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(0, len(timestamps_per_player)):
        player = timestamps_per_player[i]
        time = []
        for j in range(1, len(player)):
            time.append(player[j] - player[j-1])
        time_per_word.append(time)

    return match(words, time_per_word)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for j in word_indices:
        ...
        for i in player_indices:
            if time(match, i, j) < time(match, index_i, j):
                index_i = i
        result[index_i].append(get_word(match, j))
