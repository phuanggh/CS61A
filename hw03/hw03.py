...
def digit_distance(n):
    ...
    if n < 10:
        ...
        last, second = n % 10, n // 10 % 10
        return abs(last - second) + digit_distance(n // 10)
...
def interleaved_sum(n, odd_term, even_term):
    ...
    "*** YOUR CODE HERE ***"
    def addOdd(n):
        return odd_term(n) + addEvent(n-1)
    
    def addEvent(n):
        if n == 0:
            return 0
        else:
            return even_term(n) + addOdd(n-1)
...
def count_coins(total):
    ...
    def count(total, max):
        ...
        if max == None:
            ...
            return count(total - max, max) + count(total, next_smaller_coin(max)) 
